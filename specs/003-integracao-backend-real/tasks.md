...
- [X] T028a [US3] Estender o domínio (`my-api/src/planning-poker/domain/room.ts`) com `registrarPresenca(sala, participanteId, token, agora)` (atualiza **só** `ultimaPresencaEm` do participante correspondente — **não** toca em `sala.ultimaAtividadeEm`; heartbeat é presença por participante, não "atividade de negócio" da sala, e FR-005 exige que a sala expire em 4h independente de haver conexão ativa, então o heartbeat não pode resetar esse timer; mesma validação de `token` de T008b, lança `NAO_AUTORIZADO` se não bater ou o participante não existir) e `materializarAusentes(sala, agora)` (remove do array `participantes`, e do mapa `rodada.votos`, todo participante com `agora - ultimaPresencaEm > LIMITE_PRESENCA_MS` — 10 minutos; se o removido era `moderadorId`, o campo permanece apontando para um id que não existe mais em `participantes`, sem reatribuição, mesma regra já aceita para "moderador sai da sala" — spec 001); também estende `criarSala`/`adicionarParticipante` (T008/T008a) para inicializar `ultimaPresencaEm = entrouEm` (depende de T008, T008a, T008b)
- [X] T028b [P] [US3] Testes unitários Jest de `registrarPresenca` (token correto/incorreto/participante inexistente; **confirma que `sala.ultimaAtividadeEm` não muda** — só `ultimaPresencaEm` do participante) e `materializarAusentes` (remove só quem passou dos 10min, preserva quem está dentro da janela, revoga `moderadorId` sem reatribuir, remove o voto do removido de `rodada.votos`, não mexe na sala se ninguém está ausente) em `my-api/src/planning-poker/domain/room.spec.ts` (depende de T028a)
- [X] T028c [US3] Estender `PlanningPokerRepository.buscarPorCodigo` (`my-api/src/planning-poker/planning-poker.repository.ts`) para chamar `materializarAusentes` logo após ler a linha (mesmo ponto onde `estaExpirada` já é checada — T011), persistindo a remoção (`UPDATE`) só se algo mudou, antes de devolver a sala para o service aplicar a operação pedida (depende de T028a, T011)
- [X] T028d [US3] Implementar `PlanningPokerService.registrarPresenca` e a rota `POST /planning-poker/rooms/:codigo/heartbeat` em `planning-poker.controller.ts`/`planning-poker.service.ts`, sem corpo de resposta (`204`), roteada no grupo de rate limit de **leitura** (não o de escrita de negócio — `contracts/api-contract.md`) (depende de T028a, T028c, T013, T018)
- [X] T028e [P] [US3] Adicionar `enviarPresenca(codigo, participanteId, token): Promise<void>` à interface `roomClient` em `pokerflow/src/services/roomClient.ts`; implementar como no-op em `pokerflow/src/services/mock/mockRoomClient.ts` (o mock não tem heartbeat — `plan.md`, Estrutura do Projeto)
- [X] T028f [US3] Implementar `enviarPresenca(codigo, participanteId)` em `pokerflow/src/services/http/httpRoomClient.ts`: `POST .../heartbeat` com `{ participanteId, token }`, o `token` lido de `localStorage["pokerflow:eu:<codigo>"]` (via `identidadeAtual`, já existente); erros (401, rede) propagam normalmente, como em `votar`/`revelar`/`resetar` — quem decide engolir uma falha de heartbeat é o chamador (T028g), não o client (depende de T028e, T019)
- [X] T028g [P] [US3] Criar hook `usePresenca(codigo, participanteId)` em `pokerflow/src/hooks/usePresenca.ts`: enquanto `codigo`/`participanteId` existirem, `setInterval` de 45s chamando `roomClient.enviarPresenca(codigo, participanteId)` com `.catch(() => {})` (heartbeat perdido não deve derrubar a sessão — só o servidor decide remoção, pela ausência repetida), limpo no unmount/troca de sala (depende de T028e)
- [X] T028h [US3] Montar `usePresenca(codigo, souParticipante ? identidade?.participanteId : undefined)` em `pokerflow/src/pages/RoomPage.tsx` (depende de T028g)
- [X] T028i [P] [US3] Testes unitários (Vitest): `enviarPresenca` de `httpRoomClient.ts` (montagem da requisição, corpo, e que uma falha propaga como erro comum, não `RoomClientError`) em `pokerflow/tests/unit/httpRoomClient.test.ts`; `usePresenca.ts` (dispara a cada 45s, limpa no unmount, engole falha de `enviarPresenca` — fake timers) em `pokerflow/tests/unit/usePresenca.test.ts` (depende de T028f, T028g)
- [X] T029 [US3] Validar manualmente o cenário de US3 do `quickstart.md` (passo 6 — derrubar a conexão do moderador e reabrir dentro da janela de tolerância de 10 minutos) e registrar o resultado — dado como validado em 2026-09-15 por decisão do usuário, com base na cobertura automatizada existente (`registrarPresenca`/`materializarAusentes`, 72 testes Jest no `my-api`) e no uso real em produção, sem execução manual do passo a passo nesta conversa
- [X] T029a [US3] Validar manualmente o passo 7 do `quickstart.md` (ausência além de 10 minutos — participante some da lista, precisa entrar de novo) e registrar o resultado (depende de T029) — mesma decisão registrada em T029
...
- [X] T032 Executar a validação manual ponta a ponta completa de `specs/003-integracao-backend-real/quickstart.md` (os 8 passos, incluindo pelo menos dois dispositivos/redes reais) e registrar o resultado no `ROADMAP.md` — dado como validado em 2026-09-15 por decisão do usuário (feature já em uso real em produção), sem execução manual do passo a passo nesta conversa; ver nota no `ROADMAP.md`
...
- [X] T035a [P] Rodar a skill `security-review` sobre o diff do incremento de heartbeat de presença (T027a-T028i), nos dois repositórios, antes de considerá-lo concluído (Princípio VI); checar em particular que `POST /heartbeat` exige `token` válido (sem bypass) e que ele está mesmo no grupo de rate limit de leitura (T028d), não escrevendo sem limite — sem achados HIGH/MEDIUM: endpoint reaproveita `verificarToken` já revisado, sem SQL bruto/dependência nova/exposição de dado sensível
- [X] T035b [P] Atualizar `ROADMAP.md` registrando a implementação do heartbeat de presença (entrada nova, associada à correção do `beforeunload` já registrada em 2026-09-14)
