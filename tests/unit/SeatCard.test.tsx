...
import { Participante } from "../../src/types/room";

const outroParticipante: Participante = {
  id: "p-outro",
  nome: "Bruno",
  ehModerador: false,
  entrouEm: 0,
...
describe("SeatCard — sigilo do voto (FR-003/FR-004/SC-002)", () => {
  it("NUNCA renderiza no DOM o valor votado por outro participante quando revelado=false", () => {
    const { container } = render(
      <SeatCard participante={outroParticipante} voto="8" revelado={false} souEu={false} />,
    );
  ...
  it("indica apenas que o outro participante já votou, sem o valor, quando ainda oculto", () => {
    render(<SeatCard participante={outroParticipante} voto="8" revelado={false} souEu={false} />);
    expect(screen.getByLabelText("Bruno: já votou")).toBeInTheDocument();
  });

  it("indica que o participante ainda não votou (vazio)", () => {
    render(<SeatCard participante={outroParticipante} voto={undefined} revelado={false} souEu={false} />);
  ...
  it("marca claramente 'Não votou' após a revelação para quem não votou (FR-008)", () => {
    render(<SeatCard participante={outroParticipante} voto={undefined} revelado={true} souEu={false} />);
    expect(screen.getByText("Não votou")).toBeInTheDocument();
  });

  it("exibe o valor de outro participante depois que a rodada é revelada", () => {
    render(<SeatCard participante={outroParticipante} voto="8" revelado={true} souEu={false} />);
    expect(screen.getByText("8")).toBeInTheDocument();
  });
});

describe("SeatCard — visibilidade do próprio voto (FR-004a)", () => {
  const euParticipante: Participante = {
    id: "p-eu",
    nome: "Ana",
    ehModerador: false,
    entrouEm: 0,
  ...
  it("renderiza o valor do PRÓPRIO participante mesmo com revelado=false", () => {
    render(<SeatCard participante={euParticipante} voto="8" revelado={false} souEu={true} />);
    expect(screen.getByText("8")).toBeInTheDocument();
  });

  it("continua mostrando o próprio valor depois da revelação também", () => {
    render(<SeatCard participante={euParticipante} voto="8" revelado={true} souEu={true} />);
    expect(screen.getByText("8")).toBeInTheDocument();
  });
});
